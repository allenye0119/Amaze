...
`Amaze <https://nicolashug.github.io/Amaze/>`_ has a set of built-in
...
:func:`train_test_split() <amaze.model_selection.split.train_test_split>`
to sample a trainset and a testset with given sizes, and use the :mod:`accuracy
metric<amaze.accuracy>` of your chosing. You'll need to use the :meth:`fit()
<amaze.prediction_algorithms.algo_base.AlgoBase.fit>` method which will
train the algorithm on the trainset, and the :meth:`test()
<amaze.prediction_algorithms.algo_base.AlgoBase.test>` method which will
...
<amaze.dataset.DatasetAutoFolds.build_full_trainset>` method which will
build a :class:`trainset <amaze.Trainset>` object:
...
<amaze.prediction_algorithms.algo_base.AlgoBase.predict>` method.  Let's say
...
.. note::
    ...
    The :meth:`predict()
    <amaze.prediction_algorithms.algo_base.AlgoBase.predict>` uses **raw**
...
`Amaze <https://nicolashug.github.io/Amaze/>`_ has a set of  builtin
...
<amaze.reader.Reader>` object for `Amaze
<https://nicolashug.github.io/Amaze/>`_ to be able to parse the file or the
dataframe.

.. _load_from_file_example:

- To load a dataset from a file (e.g. a csv file), you will need the
  :meth:`load_from_file() <amaze.dataset.Dataset.load_from_file>` method:
  ...
  class <amaze.reader.Reader>` documentation.
...
- To load a dataset from a pandas dataframe, you will need the
  :meth:`load_from_df() <amaze.dataset.Dataset.load_from_df>` method. You
  will also need a :class:`Reader<amaze.reader.Reader>` object, but only
...
<amaze.model_selection.validation.cross_validate>` function that does all
...
:meth:`test()<amaze.prediction_algorithms.algo_base.AlgoBase.test>` method
...
The design of Amaze's cross-validation tools is heavily inspired from the
...
and test files (u1.base, u1.test ... u5.base, u5.test). Amaze can handle
this case by using a :class:`amaze.model_selection.split.PredefinedKFold`
object:
...
<amaze.model_selection.validation.cross_validate>` function reports accuracy
...
:class:`GridSearchCV <amaze.model_selection.search.GridSearchCV>` class
...
<amaze.prediction_algorithms.matrix_factorization.SVD>` algorithm.
...
.. note::
    ...
    :class:`KNNBaseline <amaze.prediction_algorithms.knns.KNNBaseline>`
    algorithm:
...
Amaze can also be used from the command line, for example:

.. code::

    amaze -algo SVD -params "{'n_epochs': 5, 'verbose': True}" -load-builtin ml-100k -n-folds 3
