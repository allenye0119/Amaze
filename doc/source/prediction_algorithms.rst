...
Amaze provides a bunch of built-in algorithms. All algorithms derive from
the :class:`AlgoBase <amaze.prediction_algorithms.algo_base.AlgoBase>` base
class, where are implemented some key methods (e.g. :meth:`predict
<amaze.prediction_algorithms.algo_base.AlgoBase.predict>`, :meth:`fit
<amaze.prediction_algorithms.algo_base.AlgoBase.fit>` and :meth:`test
<amaze.prediction_algorithms.algo_base.AlgoBase.test>`). The list and
details of the available prediction algorithms can be found in the
:mod:`prediction_algorithms <amaze.prediction_algorithms>` package
documentation.

Every algorithm is part of the global Amaze namespace, so you only need to
import their names from the Amaze package, for example: ::
...
:func:`pearson_baseline <amaze.similarities.pearson_baseline>` similarity.
...
- ``'name'``: The name of the similarity to use, as defined in the
  :mod:`similarities <amaze.similarities>` module. Default is ``'MSD'``.
...
.. seealso::
    The :mod:`similarities <amaze.similarities>` module.
