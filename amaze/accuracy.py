...
def rmse(predictions, verbose=True):
    ...
    mse = np.mean([float((true_r - est)**2)
                   for (_, _, true_r, est, _) in predictions if true_r > 0])
    rmse_ = np.sqrt(mse)
...
def mape(predictions, verbose=True):
    ...
    Returns:
        The Mean Absolute Percentage Error of predictions.
    ...
    mape_ = np.mean([float(abs(true_r - est) / true_r)
                    for (_, _, true_r, est, _) in predictions if true_r > 0])

    if verbose:
        print('MAPE:  {0:1.4f}'.format(mape_))
